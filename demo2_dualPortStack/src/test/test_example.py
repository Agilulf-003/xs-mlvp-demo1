...
class SinglePortDriver: # driver, connect to the dut and the model
    class Status(Enum):
        IDLE = 0
        WAIT_REQ_READY = 1
        WAIT_RESP_VALID = 2
    ...
    def __init__(self, dut, model: StackModel, port_dict): #port_dict, ports list of dut
        self.dut = dut
        self.model = model
...
#after clock rise, run call back fuction#
    stack.StepRis(port0.step_callback)  #picker api #只需使用我们的Step()方法，就可以同时更新clk和电路状态。 #
    stack.StepRis(port1.step_callback)  #https://open-verify.cc/mlvp/docs/env_usage/picker_usage/
